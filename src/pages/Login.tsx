...
export function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/users/login/', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ tr_number: tr_number, password }),
      });
      ...
      navigate(data.role === 'admin' ? '/schedule' : '/schedule');
    } catch (error) {
      console.error(error);
      toast.error('Invalid TR number or password');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="w-full min-h-screen bg-gray-950 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-gray-900 rounded-lg p-8 border border-gray-800">
        <div className='text-center'><PageHeader title="Study Planner" /></div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="Enter your TR number"
              required
            />
          </div>
          <div>
