...
export function Calendar({
  ...
  return <div>
      ...
      <div className="grid grid-cols-7 gap-2 mb-2">
        {DAY_HEADERS.map(day => <div key={day} className="text-center font-semibold text-white py-2 bg-gray-800 rounded-lg">
            {day}
          </div>)}
      </div>
      {/* Calendar grid */}
...
function CalendarCell({
  ...
  return <div ref={setNodeRef} className={`min-h-32 rounded-lg p-3 border transition-colors ${isDisabled ? 'bg-gray-900 border-gray-800 opacity-50 cursor-not-allowed' : isOver ? 'bg-gray-800 border-blue-500' : 'bg-gray-800 border-gray-700'} ${!day.isCurrentMonth ? 'opacity-60' : ''}`}>
      <div className="mb-2">
